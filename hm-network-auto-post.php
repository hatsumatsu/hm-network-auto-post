...
class HMNetworkAutoPost {
	 ...
	 */
	public function copy_post( $source_post_id, $post ) {
	...
	public function setPostThumbnail( $source_site_id, $source_post_id, $target_site_id, $target_post_id ) {
		...
		if( $source_post_thumbnail_id = get_post_thumbnail_id( $source_post_id ) ) {
			...
			if( array_key_exists( $target_site_id, $thumbnail_relations ) ) {
				$this->write_log( 'Thumbnail relation found: ' . $thumbnail_relations[$target_site_id] );
	 ...
	 * @param int $target_post_id post ID of target post
	 */
	public function setMeta( $source_site_id, $source_post_id, $target_site_id, $target_post_id ) {
		if( $this->settings[get_post_type( $source_post_id )]['meta-relations'] ) {
			foreach( $this->settings[get_post_type( $source_post_id )]['meta-relations'] as $key ) {
				$value = get_post_meta( $source_post_id, $key, true );
				if( $value ) {									
